...
then
  ...
  if [ "$1" = "webserver" ]; then
    ...
    $CMD initdb
    echo "Add static variables"
    $CMD variables --set forecast-date 2018-02-03
    $CMD variables --set market TEST
    $CMD variables --set build_type forecast
    $CMD variables --set new_market NAT2
    exec $CMD webserver
  else
    sleep 10
    exec $CMD "$@"
